...
class BaseTrajectoryVisualizer(ABC):
    def __init__(self, image_names, state):
        super().__init__()
    ...
    def add(self, images, state):
        self.images = images

    def reset(self, image_names, state):
        self.__init__(image_names, state)
    
    def create_plot(self):
        plt.ion()
        self.fig = plt.figure(figsize=(5 * self.num_images, 5))
...
class AbsoluteTrajectoryVisualizer(BaseTrajectoryVisualizer):
    def __init__(self, image_names, ee_state):
        super().__init__(image_names, ee_state)

    def add(self, images, state):
        super().add(images, state)
...
class MultiTrajectoriesVisualizer:
    ...
    def add(self, images, states):
        self.images = images
        states = [states[i:i + 7] for i in range(0, len(states), 7)]
        for name, state, visualizer in zip(self.names, states, self.visualizers):
            visualizer.add(images, state)
    
    def reset(self, image_names, names, visualizers):
        self.__init__(image_names, names, visualizers)
    ...
    def plot(self, names=None):
        ...
        ax = self.fig.add_subplot(1, self.num_images + 1, self.num_images + 1, projection='3d')
        for name, visualizer in zip(self.names, self.visualizers):
            if names is not None:
                if name not in names:
                    continue
            
            positions = np.array(visualizer.trajectory['positions'])
            ax.plot(positions[:, 0], positions[:, 1], positions[:, 2], 
            ...
            n_points = len(positions)
            step = max(1, n_points // 20) 
            euler_angles = np.array(visualizer.trajectory['euler_angles'])
...
def get_visualizer(image_names, init_state, control_mode, multi_arm=False) -> BaseTrajectoryVisualizer:
    if control_mode == 'ee_absolute':
        visualizer = AbsoluteTrajectoryVisualizer(image_names, init_state)
    elif control_mode == 'ee_delta_gripper':
        visualizer = DeltaGripperTrajectoryVisualizer(image_names, init_state)
    else:
